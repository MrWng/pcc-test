...
@Component({
  ...
  providers: [BasicDataTemplateManageService, WbsTabsService, DynamicWbsService, AddSubProjectCardService, CommonService],
})
export class BasicDataTemplateManageComponent
  ...
  initData(): void {
    ...
    if (this.group.value?.parameterData) {
      this.wbsService.project_no = this.group.value?.parameterData?.project_template_info[0].project_template_no;
...
*/
  getTaskInfo(): void {
    this.commonService.getTaskInfo(this.wbsService.project_no, Entry.maintain).subscribe(
      (value) => {
  ...
  saveToTemplate() {
    ...
    const url = this.wbsService.project_no ? 'bm.pisc.project.template.update' : 'bm.pisc.project.template.create';
    this.commonService.getInvData(url, params).subscribe((res) => {
      if (res && res.code === 0) {
        ...
        this.wbsService.projectInfo.project_type_no = this.validateForm.get('project_type_no').value ?? null;
        const info = this.isEdit ? 'dj-default-保存成功！' : 'dj-default-模板创建成功！';

        if (!this.isEdit) {
          this.validateForm.get('project_template_no').disable();
        }
        if (this.isEdit && res.data && res.data?.project_template_info
