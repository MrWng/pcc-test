...
import { DwFormArray, DwFormControl, DwFormGroup } from '@athena/dynamic-core';
...
export class CommonService {
  ...
  // 推送问题处理相关
  async submitDispatchForQuestion(params: any): Promise<void> {
    const DwUserInfo = JSON.parse(sessionStorage.DwUserInfo ?? '{}');
    const { taskWithBacklogData } = this.content?.executeContext ?? {};
    const workitemListItem = taskWithBacklogData?.backlog[0]?.workitemList[0];
    const submit_params = {
  ...
  translatePccWord(word: string) {
    return this.translateService.instant('dj-pcc-' + word);
  }
