...
export class ProjectTableService {
   ...
   * @param personList
   */
  handelData(enterParams: any, updateValues, personList?) {
    ...
    if (params.liable_person_code_key) {
      personList.forEach((person) => {
        ...
        if (
          ...
          params.liable_person_role_name = role_name || liable_person_code_key[0];
        }
      });
    } else {
      ...
      params.task_member_info = params.task_member_info_key.map((info_key) => {
        ...
        personList.forEach((person) => {
          ...
          if (
            ...
            info = {
              executor_role_name: role_name ? role_name : '',
