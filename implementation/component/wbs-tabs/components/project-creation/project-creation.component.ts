...
import { DwFormControl, cloneDeep, dayjs, isEmpty } from '@athena/dynamic-core';
...
export class ProjectCreationComponent implements OnInit, OnChanges, OnDestroy {
          ...
          this.openWindowService.openWindow(
            ...
            (data: Array<any>) => {
              ...
              if (data[0].effective_date && data[0].expiration_date) {
                const startTime = moment(this.validateForm.getRawValue().plan_start_date).format(
                  'YYYY-MM-DD'
                );
                const endTime = moment(this.validateForm.getRawValue().plan_finish_date).format(
                  'YYYY-MM-DD'
                );
                this.changeEndTime(startTime, endTime);
              } else {
              ...
              if (this.validateForm.getRawValue().actionCode === '2') {
                this.validateForm
                  ...
                  .setValue(moment(data[0].effective_date).format('YYYY-MM-DD'));
                this.commonService
  ...
  choosePotentialProjectNo() {
    if ((this.validateForm.value.project_property !== '20')
