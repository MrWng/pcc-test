...
export class PersonInChargeComponent extends AcBaseCellComponent implements OnInit, OnChanges {
    ...
    if(Array.isArray(this.value) && this.value?.length > 0){
      this.value.forEach((node, index)=>{
        ...
        personList.forEach(person=>{
          const { role_name, role_no, department_name, department_no, employee_name, employee_no } = person;
          const person_code_key = node.split('/');
          if((person_code_key[0] === role_no || person_code_key[0] === role_name) &&
            person_code_key[1] === department_no && person_code_key[2] === employee_no){
            isInnerList += 1;
          }
        });
        if(isInnerList === 0){
      ...
      personList.forEach(person=>{
        ...
        if((liable_person_code_key[0] === role_no || liable_person_code_key[0] === role_name) &&
          liable_person_code_key[1] === department_no && liable_person_code_key[2] === employee_no){
          isInnerList += 1;
        }
      });
      if(isInnerList === 0){
  ...
  getDropdownClassName(){
    if(!this.isCanSelectParentNode){
      return 'cus-pcc-add-role';
    }else{
      return 'cus-pcc-add-role1';
    }
  }

  displayWith = (node: NzTreeNode) => {
    return `${node.title}${node.origin?.department_name ? ' ' + node.origin?.department_name : ''}${node.origin?.role_name ? ' ' + node.origin?.role_name : ''}`;
  }
