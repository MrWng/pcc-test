...
export class ManualWorkReportingComponent extends DynamicFormControlComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    console.log('输出当前组件对象:  ',this); // 输出当前组件对象
  }
  ...
  getRowTData() {
    ...
    const personList = this.dynamicGroup.get('manualWorkReporting')?.value;
    const clone_work_hours_list = cloneDeep(personList).filter((item) => item.work_hours !== null);
