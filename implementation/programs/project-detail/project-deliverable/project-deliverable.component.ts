...
import { DynamicFormControlComponent, DynamicFormControlLayout, DynamicFormLayout, DynamicFormLayoutService, DynamicFormValidationService, cloneDeep, } from '@athena/dynamic-core';
...
export class ProjectDeliverableComponent extends DynamicFormControlComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    } else if (this.group?.value?.category && (this.group?.value?.category === 'manualAssignmentDelivery_PLM')) {
      this.deliverable_plm = cloneDeep(this.group?.value);
      if (Object.keys(this.deliverable_plm).length) {
        this.deliverable_plm_attachment = this.deliverable_plm['attachment'] ?? this.deliverable_plm['attachment1'];
  ...
  downLoad(file: any): void {
    ...
    this.projectDeliverableService.project_task_delivery_enclosure_data_get(params).then((doc_info) => {
      if (doc_info?.code === '0' && doc_info?.deliverable_no && doc_info?.token && doc_info?.deliverable_attachment_link) {
