...
import { FROM_GROUP_KEY, getColumnDefs } from './config';
import { AcAcTableCellComponent } from '@app-custom/ui/ac-table-cell';
...
export class TaskDetailService {
  ...
  getColumnDefs(
    ...
    return getColumnDefs(customFilter, translateService, componentParams);
  }
  ...
  getWorkReport(params: any, eoc_company_id) {
    // return this.http
    ...
    return this.commonService
      .getInvData('wo.work.report.get', params, eoc_company_id)
      .pipe(
        map((res: any) => {
  ...
  transformParams(params) {
    ...
    const result = {};
    const needSetBoolean = ['is_doc_date', 'is_confirm_date', 'is_project_no', 'is_task_no'];
    Object.keys(params || {}).forEach((key) => {
      if (!keyMap[key]) {
        return;
      }
      result[keyMap[key]] = needSetBoolean.includes(key) ? params[key].toString() : params[key];
    });
    return result;
  }
}
