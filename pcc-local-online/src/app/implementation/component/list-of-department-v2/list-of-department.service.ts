...
@Injectable()
export class ListOfDepartmentV2Service {
  uibotUrl: string;
  eocUrl: string;
  ...
  getRoleList(): Promise<any> {
    return new Promise((resolve, reject) => {
      this.commonService.queryCatalog().subscribe(
        (res) => {
          ...
          catalogList.unshift({
            id: '#%#@%##',
            name: this.translateService.instant('dj-pcc-无角色'),
          });
          resolve(catalogList);
        },
        (err) => {
  ...
  getSelectList(rolelist: any, disabledList: any, list: any): any {
    list.forEach((target) => {
      ...
      target.role_name = target.role_name
        ? target.role_name
        : this.translateService.instant('dj-pcc-无角色');
      ...
      // 禁用已选人员中的被关联的人员
      target.disableCheckbox = disabledList.find(
        (o) => o.role_no === role_no && o.employee_no === empId && o.department_no === deptId
      )
        ? true
        : false;
    });
          ...
          if (roleItem) {
            ...
            if (node.origin.role_no === '#%#@%##') {
              personTree.unshift({
                ...
                deptGroups: [cloneDeep(node.origin)],
              });
            } else {
              personTree.push({
                role_no: node.origin.role_no,
