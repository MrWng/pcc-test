...
import { ListOfDepartmentV2Service } from './list-of-department.service';
...
export class ListOfDepartmentV2Component implements OnInit, OnChanges {
  ...
  check_data: String = '';
  expendedNodeKeys = Object.create(null);
  ...
  constructor(
    ...
    public listOfDepartmentService: ListOfDepartmentV2Service,
   ...
   */
  getSelectMemberList() {
    this.expendedNodeKeys = Object.create(null);
    const projectNo =
      this.wbsService.project_no ||
  ...
  checkBoxChange(event: NzFormatEmitEvent): void {
    ...
    // if (has(origin, 'deptId') && has(origin, 'role_no')) {
    //   this.expendedNodeKeys[`${origin['role_no']}_${origin['deptId']}`] = false;
    //   this.expendedNodeKeys[origin['role_no']] = false;
    // } else {
    //   this.expendedNodeKeys[origin['role_no']] = false;
    // }
  ...
  trackByDept(index, item) {
    return item.deptId;
  }
  trackByPerson(index, item) {
    return item.employee_no;
  }
  expendAndCollapse(item, dom, key) {
    let expendedNodeKey = '';
    if (key === 'deptId') {
      expendedNodeKey = `${item.role_no}_${item.deptId}`;
    } else {
      expendedNodeKey = `${item.role_no}`;
    }
    this.expendedNodeKeys[expendedNodeKey] = !this.expendedNodeKeys[expendedNodeKey];
