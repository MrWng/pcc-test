...
@Injectable()
export class ProjectTableOptionsService {
   ...
   *
   */
  getAddButtonPermissions(taskCard: any, schedule_status, noHasChildWithTaskProportionLessOne: boolean): boolean {
    const project_status = this.wbsService?.projectInfo?.project_status;
    return schedule_status === '1' && (
      project_status === '10' ||
      (project_status === '30' && noHasChildWithTaskProportionLessOne && taskCard.old_task_status === '10') ||
      (project_status === '30' && noHasChildWithTaskProportionLessOne && taskCard.old_task_status === '20' && taskCard?.children?.length)
    );
  }
  ...
  */
  async deleteAssistTaskDetail(assist_task_detail_info: any): Promise<any> {
    const res: any = await this.commonService.getInvData('bm.pisc.assist.task.detail.delete.process', assist_task_detail_info).toPromise();
  ...
  async deleteTaskChangeInfo(params: any): Promise<any> {
    const res: any = await this.commonService.getInvData('bm.pisc.project.change.task.detail.delete', params).toPromise();
    return res;
  }
  ...
  */
  async getProjectInfo(): Promise<any> {
    const res: any = await this.commonService.
      getInvData('bm.pisc.project.get', { project_info: [{ project_no: this.wbsService.project_no }] }).toPromise();
