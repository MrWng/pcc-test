  ...
  async personInCharge(item, isdeletTem?: boolean) {
    ...
    if (!item) {
      ...
      if (task_member_info?.length) {
        // 将【负责人】已选项在【执行人】已选项中，剔除
        const result = task_member_info.filter(info => !info.includes(';' + item.id));
        this.addSubProjectCardService.validateForm.get('task_member_info').setValue(result);
      }
