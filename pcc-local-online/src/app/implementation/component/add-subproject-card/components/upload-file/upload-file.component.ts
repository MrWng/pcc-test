...
@Component({
  ...
  providers: [DmcService, MyUploadService, AthMessageService],
})
export class UploadFileComponent implements OnInit {
  ...
  beforeUpload = (file: NzUploadFile, fileList: UploadFile[]): boolean => {
    if (
      ...
      this.attachmentData.some((item) => item.name === file.name)
    ) {
    ...
    if (this.addSubProjectCardService.filterSize(file.size) > 50) {
      this.messageService.error(this.translateService.instant(`dj-default-上传文件大小不超过50MB`));
      return false;
    } else {
      return true;
    }
  ...
  previewFile = (file: UploadFile): void => {
    ...
    if (this.addSubProjectCardService.isAssetTypeAnImage(ext)) {
      ...
      this.attachmentData.forEach((fileItem): void => {
        if (
          this.addSubProjectCardService.isAssetTypeAnImage(
            fileItem.name.substr(fileItem.name.lastIndexOf('.') + 1)
          )
        ) {
          imageList.push({
            ...fileItem,
