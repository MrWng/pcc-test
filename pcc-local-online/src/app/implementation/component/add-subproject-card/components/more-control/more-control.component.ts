...
export class MoreControlComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.addSubProjectCardService.taskTemplateNo && Object.keys(this.addSubProjectCardService.currentCardInfo).length) {
      ...
      const taskCategory = this.taskTemplateInfo?.task_category ?? this.addSubProjectCardService.currentCardInfo.task_category;
      if (['APC', 'APC_O'].includes(taskCategory) && !this.addSubProjectCardService.isPreview) {
        const enableColumn = ['doc_type_no', 'doc_no', 'seq', 'type_condition_value', 'sub_type_condition_value'];
        enableColumn.forEach((column) => { this.validateForm.get(column).enable(); });
      }
    }
  ...
  setDocTypeNoList(taskTemplate: any): void {
    ...
    if (!docType) { return; }
    // 单别
    if (['APC', 'APC_O'].includes(taskTemplate.task_category)) {
      const paramsDoc = [{
        business_region: '010',
        business_property: 'A3',
        manage_status: 'Y',
        eoc_company_id: taskTemplate.eoc_company_id || '',
        eoc_site_id: taskTemplate.eoc_site_id || '',
        eoc_region_id: taskTemplate.eoc_region_id || '',
      }];
      // 模板页面，平台没有推送executeContext.businessUnit信息，需要特别处理
      const docEcoMap = taskTemplate.eoc_company_id;
      this.commonService
        .getInvData('bm.basc.business.type.configure.get', { general_business_type_info: paramsDoc }, docEcoMap)
        .subscribe((res: any): void => {
          this.changStatus.emit({ type: 'loading', value: false });
          this.docTypeNoList = res.data?.business_type_general?.map(item => {
            item.docTypeLabel = item.business_type_no + item.business_type_name;
            item.doc_type_name = item.business_type_name;
            item.doc_type_no = item.business_type_no;
            return item;
          });
          this.changeRef.markForCheck();
        });
