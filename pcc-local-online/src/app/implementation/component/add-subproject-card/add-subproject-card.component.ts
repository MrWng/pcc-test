...
import { NzModalService, OpenWindowService } from '@athena/dynamic-ui';
...
export class AddSubprojectCardComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  get isApproveStatus() {
    return ['PLM', 'PLM_PROJECT', 'ASSC_ISA_ORDER', 'PCM', 'PO_NOT_KEY'].includes(
  ...
  async ngOnInit(): Promise<void> {
    console.log('子项开窗-sourceRealy：', this.sourceRealy);
  ...
  monitorFormChange(): void {
    this.validateForm?.valueChanges.subscribe((form) => {
      ...
      this.setTaskDisabled(form); // 模板，负责人、执行人
  ...
  async clickSubmit(): Promise<any> {
    ...
    if ((this.source === Entry.card)
      ...
      && !this.addSubProjectCardService.cannotModified(this.addSubProjectCardService.currentCardInfo.task_no)) {
      this.modalService.info({
        nzTitle: this.translateService.instant('dj-c-提示'),
        nzContent: this.translateService.instant('dj-pcc-任务正在下发或更新中，不可变更，请1分钟后再试！'),
  ...
  callChooseTaskTemplate(selectOption): void {
    ...
    // [需要签核]管控
    if (
      ['PLM', 'PLM_PROJECT', 'ASSC_ISA_ORDER', 'PCM', 'PO_NOT_KEY'].includes(this.taskCategoryType)
    ) {
  ...
  setMoreControlValues(taskCategory: string): any {
      ...
      CONTROL_ITEM_LIST2.forEach((key: string): void => {
        ...
        if (
          ...
          if (
            ...
            ['MOOP', 'SFT'].includes(taskCategory)
          ) {
            this.validateForm.get(key).enable();
          } else {
    ...
    if (['MOOP'].includes(this.taskTemplateInfo?.task_category)) {
      if (is_need_doc_no) {
  ...
  taskCategoryForApc(taskCategory: string): any {
    ...
    if (
      ...
      is_need_doc_no = currentCardInfo?.is_need_doc_no;
    } else if (
      this.taskTemplateInfo &&
      Object.keys(this.taskTemplateInfo).find((item) => item === 'is_need_doc_no')
    ) {
    ...
    if (!this.addSubProjectCardService.isPreview) {
      const enableList = ['doc_type_no', 'doc_no', 'seq', 'type_condition_value', 'sub_type_condition_value'];
      enableList.forEach((element) => {
        this.addSubProjectCardService.validateForm.get(element).enable();
      });
      if (currentCardInfo?.task_category !== taskCategory) {
        enableList.forEach((element) => {
          this.addSubProjectCardService.validateForm.get(element).patchValue('');
        });
      }
