...
import { Subscription, combineLatest, forkJoin, zip } from 'rxjs';
import {
...
import { DynamicComponentRenderForAthTableService, NzModalService } from '@athena/dynamic-ui';
...
export class ProjectTableComponent implements OnInit, OnChanges, OnDestroy {
    ...
    if (changes.pageDatas?.currentValue) {
      if (this.source === Entry.card) {
        this.commonService
          ...
          .subscribe(
            (resChange: any): void => {
              if (!resChange.data?.project_info[0]?.check_result || !this.editable) {
      ...
      changes.personList?.currentValue &&
      JSON.stringify(changes.personList.currentValue) !==
        JSON.stringify(changes.personList.previousValue)
    ) {
  ...
  getRules() {
    return [
      ...
      },
      {
        ...
        trigger: {
          condition: `!!data.liable_person_code_key && !!data.task_member_info_key && data.task_member_info_key.some(item=>{return item.includes(data.liable_person_code_key.split('/')[2])})`,
          type: 'sync',
          point: 'default',
        },
        key: 'custom',
      ...
      },
      {
        ...
        condition: 'true',
        scope: 'EDIT',
        errorMessage: this.translateService.instant(`dj-default-开始时间必须早于结束时间`),
        trigger: {
          condition: `(function (currentControl, utils, moment) {
            if(!currentControl.valueIsChanged) {
              return false
            }
            const { plan_finish_date, plan_start_date, workload_qty } = currentControl.parent.getRawValue();
            if(workload_qty === 0) {
  ...
  initData = (pageDatas) => {
    ...
    this.columnDefs = [
          ...
          compProps: {
            ...
            insertRow: (e, params) => {
              ...
              if (
                ...
                //   this.translateWordPcc('尾阶任务已开始或者任务协同中，不能新增下阶')
                // );
                return;
              }
      ...
      },
      {
        ...
        resizable: true,
        filter: true,
        valueGetter: function (noMaintain, param) {
          if (param.data && param.data.hasOwnProperty('liable_person_code_key')) {
            const keys = param.data.liable_person_name_key.split(' ').reverse();
            return noMaintain ? keys.join('/') : keys.slice(1).join('/');
          }
        }.bind(this, this.source !== Entry.maintain),
        cellRendererParams: {
      ...
      },
      {
        ...
        resizable: true,
        valueGetter: function (noMaintain, param) {
          if (param.data && param.data.hasOwnProperty('task_member_info_key')) {
            const res = param.data.task_member_info_name.map((item) => {
              const keys = item.split(' ').reverse();
              return noMaintain ? keys.join('/') : keys.slice(1).join('/');
            });
      ...
      },
      {
        ...
        valueGetter: (param) => {
          ...
          if (param.data) {
            return map[param.data.workload_unit];
          }
        },
        cellRendererParams: {
          models: [{}],
          componentType: 'selectCell',
          compProps: {
            selectSettings: {
              nzShowSearch: true,
              nzDisabled: true,
      ...
      },
      {
        ...
        resizable: true,
        valueGetter: (param) => {
          if (param.data && param.data.hasOwnProperty('is_approve')) {
            return param.data.is_approve ? '是' : '否';
          }
        },
        cellRendererParams: {
          /**
      ...
      },
      {
        ...
        filter: true,
        valueGetter: (param) => {
          if (param.data && param.data.hasOwnProperty('is_attachment')) {
            return param.data.is_attachment ? '是' : '否';
          }
        },
        cellRendererParams: {
          /**
    ...
    this.transformData(pageDatas).then((r) => {
      ...
      this.projectSubscribe = combineLatest(
        this.projectFormGroup.statusChanges,
        this.projectFormGroup.valueChanged
      )
        .pipe(debounceTime(1000))
        .subscribe(([validInfo, res]) => {
          if (validInfo !== 'PENDING') {
            this.tableChange(res);
          }
        });
  ...
  getUpperLevelTaskPath(upper_level_task_no, path) {
    const upperTask = this.pageDataList.filter((item) => item.task_no === upper_level_task_no)[0];
    if (upperTask) {
  ...
  async checkValue(parentControl: any, schema?: string): Promise<void> {
    ...
    const { task_name, workload_qty, plan_work_hours } = updateValues;
    if (!task_name?.trim()) {
      return;
    }
    if (!this.dateCheck(parentControl, schema)) {
    ...
    if (!plan_work_hours) {
      updateValues.plan_work_hours = 0;
    }
      ...
      if (this.source === Entry.collaborate || this.source === Entry.projectChange) {
        ...
        this.handelData({
          status:
            ...
            Number(this.wbsService.projectInfo?.project_status) === 30,
        });
      } else {
        // 项目进行中，编辑任务，1分钟管控，校验
        if (
          this.source === Entry.card &&
          parentControl?.value?.task_no &&
          this.wbsService.projectInfo.project_status === '30' &&
          !this.addSubProjectCardService.cannotModified(parentControl.value.task_no)
        ) {
          this.modalService.info({
            nzTitle: this.translateService.instant('dj-c-提示'),
            nzContent: this.translateService.instant(
              'dj-pcc-任务正在下发或更新中，不可变更，请1分钟后再试！'
            ),
      ...
      if (this.source === Entry.collaborate) {
        ...
        res = await this.projectTableService.taskBaseInfoUpdate(updateParams, () => {
          if (
            ...
            // 项目进行中，编辑任务，1分钟管控，记录修改任务卡的时间戳
            this.addSubProjectCardService.recordModifiedTimeStamp(params.task_no);
          }
          this.loading = false;
          this.noTriggerChange = false;
          this.changeRef.markForCheck();
  ...
  getFirstLevelTaskCard(firstLevelTaskNo) {
    return this.pageDataList.filter((data) => data.task_no === firstLevelTaskNo)[0];
  }

  // 获取上阶任务卡信息
  getParentTime(upper_level_task_no) {
    return this.pageDataList.filter((data) => data.task_no === upper_level_task_no)[0];
  }
