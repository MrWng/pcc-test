...
export class TaskDetailComponent implements OnInit {
  @ViewChild('contentTaskDetail') contentTaskDetail;
  state: string = '1';
  baseData: Array<any> = [];
  list: Array<any>;
  ...
  constructor(
    ...
    public dynamicCustomizedService: DynamicCustomizedService
  ) {
  ...
  chengyiTaskDetailDynamicModel;
  initIndividualCop() {
    const curTenantId = JSON.parse(window.sessionStorage.getItem('DwUserInfo')).tenantId;
    // const curTenantId = 'uc72008898';
    this.chengyiTaskDetailDynamicModel = this.dynamicCustomizedService.getComponent({
      tenantIdComponent: curTenantId + '-task-detail',
      _component: this,
    });
  }
  ...
  async loadData(): Promise<any> {
    ...
    // this.getT100(this.wbsService.taskDetail.task_category);
    if (
      this.wbsService.taskDetail.task_category === 'MOMA' &&
      this.dynamicCustomizedService.isChengYiIndCase
    ) {
      this.initIndividualCop();
      return;
    }
    if (['ODAR'].includes(this.wbsService.taskDetail.task_category)) {
      ...
      if (
        'APC_O' === taskDetail.task_category &&
        taskDetail.is_project_no
      ) {
        res_flag = true;
      }
      if ('APC' === taskDetail.task_category && taskDetail.is_need_doc_no && taskDetail.doc_type_no && taskDetail.doc_no) {
        res_flag = true;
      }
      if (
        'APC' === taskDetail.task_category &&
        taskDetail.is_project_no
      ) {
        res_flag = true;
      }
  ...
  // getT100(category) {
  //   if (['POPUR'].includes(category)) {
  //     this.taskDetailService.getTenantProductOperationList();
  //   }
  // }
  ...
  selectedIndexChange() {
    this.state = (this.tabSelectedIndex + 1).toString();
    this.buildListData();
    this.changeRef.markForCheck();
  }
}
