...
export class WbsHeaderComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    console.log('wbsHeader-sourceRealy: ', this.sourceRealy);
    // eslint-disable-next-line no-unused-expressions
    this.source !== Entry.maintain && this.getProjectIdForQueryApprove();
  ...
  // [项目计划维护]，项目启动签核状态
  get isProjectStartSignOffForCard() {
    const { approve_status, project_status } = this.wbsService.projectInfo;
    return this.sourceRealy === Entry.card && approve_status === 'N' && project_status === '20';
  }

  // [项目计划]，项目启动签核状态
  get isProjectStartSignOffForPlan() {
    const { approve_status, project_status } = this.wbsService.projectInfo;
    return this.sourceRealy === Entry.plan && approve_status === 'N' && project_status === '20';
  }
  ...
  getProjectIdForQueryApprove(): void {
    ...
    if (this.wbsService.projectInfo?.project_status === '20') {
      // [项目计划维护】，项目启动签核
      tmpId = 'projectCenterConsole_userProject';
      actTmpId = 'maintenanceProjectApproval_DTD';
    }
    this.getProcessForApprove(bkInfo, tmpId, actTmpId);
  }
    ...
    this.wbsService
      ...
      .subscribe((res: any): void => {
        ...
        if (this.checkReviewTaskPlanned && this.source === Entry.maintain) {
          this.commonService
            .checkTask(this.wbsService.project_no, '', this.source)
  ...
  getNewStatus() {
    if (this.delayType === 1) {
      ...
      this.commonService.getInvData('bm.pisc.project.get', params).subscribe(
        (res: any): void => {
   ...
   */
  lookGatte(): void {
    this.wbsService.changeType('gatte');
   ...
   */
  lookPert(): void {
    this.wbsService.changeType('pert');
  ...
  createCollaborativePlan(): void {
    ...
    if (!tempList || !tempList.length) {
      this.collaborativeHandleCancel();
      return;
    }
    ...
    this.commonService
      ...
      .subscribe((res) => {
        if (res.code === 0) {
          ...
          // 解决动态新增属性，不触发子组件变更
          const pageDatas = cloneDeep(this.wbsService.pageDatas);
          tempList.forEach((listItem: any): void => {
            pageDatas.forEach((pageDatasItem: any) => {
          ...
          this.wbsService.pageDatas = pageDatas;
          ...
          this.openWindowService.openWindow(
            ...
            (resData: Array<any>) => {
              ...
              if (this.checkReviewTaskPlanned) {
                this.commonService
                  .checkTask(this.wbsService.project_no, project_template_no, this.source, '2')
                  .subscribe((resultData: any): void => {
                    const hasReviewTasks = resultData?.data?.project_info.find((item: any): any => {
                      return item.task_category === 'REVIEW';
                    });
   ...
   * @param value
   */
  async taskAfresh(value: any, callback = () => {}): Promise<any> {
    const task_generated = await this.commonService
      .getMechanismVariableList([
        {
          variableId: 'task_generated',
        },
      ])
      .toPromise()
    ...
    const params = {
      ...
      project_info: [value],
      task_name_rule: task_generated === '2' ? '2' : '1',
    };
    // 计划维护，选择【职能模板】后确定，调用API
    this.commonService.getInvData('task.afresh.create', params).subscribe(
      (res) => {
  ...
  tipContent() {
    if (this.wbsService.typeChange === 'card') {
      return this.wbsService.generationWbsTip(this.source);
    }
    return `<p class="tip-content-title">${this.translateWordPcc('PERT说明')}</p>
    <p class="tip-content-msg">${this.translateWordPcc(
      '1.颜色说明：深蓝色表示关键路径任务；淡紫色表示非关键路径任务；关键路径任务间的连接线为紫色；非关键路径任务间的连接线为灰色;'
    )}</p>
    <p class="tip-content-msg">${this.translateWordPcc('2.每个任务节点均可以拖动查看;')}</p>
    <p class="tip-content-msg">${this.translateWordPcc('3.悬停任意任务节点会同时突出相邻节点;')}</p>
    <p class="tip-content-msg">${this.translateWordPcc(
      '4.悬停任意连接线会突出该连接线的前道任务节点以及后道任务节点;'
    )}</p>
    <p class="tip-content-msg">${this.translateWordPcc(
      '5.悬停画面中间推动鼠标滚轮，可缩放画面大小;'
    )}</p>
    <p class="tip-content-msg">${this.translateWordPcc(
      '6.在有任务关系的空白区域内，按住鼠标左键拖动，整体任务关系可跟随鼠标移动;'
    )}</p>
    `;
  }
