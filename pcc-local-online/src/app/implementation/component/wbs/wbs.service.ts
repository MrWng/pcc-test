...
import { WbsCardComponent } from './components/wbs-card/wbs-card.component';
...
export class DynamicWbsService {
  ...
  projectChangeStatus: Object = {
    ...
    // 用于基础维护信息，表格栏位管控
    check_type_creation: true, // 入参：1,5
    check_type_creation_for_projectType: true, // 入参：2
  };
  ...
  needRefresh = '';
  needFullScreen: boolean = true; // 手动任务-项目信息开窗已经全屏，不需要按钮了
  TaskType = {
   ...
   * @return 项目周期
   */
  getPjPeriod(startT: string, endT: string, showEndYear = false): string {
    try {
      if (startT && endT) {
        const startDate = new Date(startT),
          ...
          endYear =
            startDate.getFullYear() === endDate.getFullYear() && !showEndYear
              ? ''
              : `${endDate.getFullYear()}/`,
  ...
  setWbsCardAboutInfoAfterRender(e) {
    // 处理当前节点
    const otherInfo = e.otherInfo,
      childrenInfo = otherInfo.children;
    if (!childrenInfo?.length) {
      return;
    }
    let connectHeight = 0;
    childrenInfo.forEach((item, index) => {
      otherInfo.cardChildeHeight +=
        item.cardChildeHeight + item.cardHeight + otherInfo.cardMarginBottom;
    });
    const last = childrenInfo[childrenInfo.length - 1];
    connectHeight =
      otherInfo.cardChildeHeight -
      last.cardChildeHeight -
      last.cardHeight +
      otherInfo.cardWireEndpointsMarginTop +
      otherInfo.cardShowAndHiddenBtnPositionBottom;
    otherInfo.connectHeight = connectHeight;
  }

  setWbsCardShowOrHideAboutInfo(item, cardInstance: WbsCardComponent) {
    let curCardInfo = cardInstance.getCardOtherInfo();
    const calcDifference = (info) => {
      const children = info.children;
      let _difference = 0;
      children.forEach((cItem) => {
        _difference += cItem.cardChildeHeight + cItem.cardHeight + info.cardMarginBottom;
      });
      return _difference;
    };
    const calcHeightByDifference = ((diff) => {
      return (info, needCalcConnectHeight = true) => {
        if (item.isChildrenshow) {
          info.cardChildeHeight += diff;
          info.connectHeight = needCalcConnectHeight
            ? info.connectHeight + diff
            : info.connectHeight;
        } else {
          info.cardChildeHeight -= diff;
          info.connectHeight = needCalcConnectHeight
    ...
    })(calcDifference(curCardInfo));
    let parentCard = cardInstance.parentWbsCardCop,
      parentOtherInfo = parentCard?.getCardOtherInfo(),
      parentOtherInfoChildLen = parentOtherInfo?.children?.length;
    calcHeightByDifference(
      curCardInfo,
      parentOtherInfo ? parentOtherInfo.children[parentOtherInfoChildLen - 1] !== curCardInfo : true
    );
    while (parentOtherInfo) {
      calcHeightByDifference(
        parentOtherInfo,
        parentOtherInfo
          ? parentOtherInfo.children[parentOtherInfoChildLen - 1] !== curCardInfo
          : true
      );
      curCardInfo = parentOtherInfo;
      parentCard = parentCard.parentWbsCardCop;
      parentOtherInfo = parentCard?.getCardOtherInfo();
      parentOtherInfoChildLen = parentOtherInfo?.children?.length;
    }
  }
  generationWbsTip(source: string = Entry.card) {
    const isTrackPages = this.isTrackPages && source === Entry.card;
    const illustrateItems = [
      this.translateWordPcc('深蓝色：表示首阶任务，不会下发任务卡执行的任务'),
      this.translateWordPcc('浅紫色：表示尾阶任务，会下发任务卡执行的任务'),
      this.translateWordPcc('渐变蓝：表示中阶任务，既不是首阶也不是尾阶，不会下发任务卡执行的任务'),
      this.translateWordPcc(
        '鼠标悬停卡片，出现浮框：完整的展示卡面呈现的内容，包括当前任务各标签状态的含义，任务增删改查等操作按钮'
      ),
    ];
    if (isTrackPages) {
      illustrateItems.pop();
      illustrateItems.push(
        this.translateWordPcc(
          '鼠标悬停卡片，出现浮框：完整的展示卡面呈现的内容，包括当前任务各标签状态的含义，查看任务信息的操作按钮'
        ),
        this.translateWordPcc('点击卡片，可以查看任务的进度明细'),
        this.translateWordPcc('进度说明')
      );
    }
    const title = this.translateWordPcc(isTrackPages ? '进度追踪wbs卡片说明' : 'wbs卡片说明');
    let content = illustrateItems.reduce(
      (acc, item, index) =>
        (acc += `<p class="tip-content-msg card-type">${index + 1}.${item}</p>`),
      ''
    );
    if (isTrackPages) {
      content += `
      <div class="wbs-tip-img-tip">
         <div class="wbs-tip-box">
           <div class="wbs-tip-icon not-started-is-overdue"></div>
            <div class="wbs-tip-text">${this.translateWordPcc('未开始已逾期')}</div>
         </div>
         <div class="wbs-tip-box">
            <div class="wbs-tip-icon not-overdue-in-progress"></div>
            <div class="wbs-tip-text">${this.translateWordPcc('进行中未逾期')}</div>
         </div>
         <div class="wbs-tip-box">
            <div class="wbs-tip-icon ongoing-is-overdue"></div>
            <div class="wbs-tip-text">${this.translateWordPcc('进行中已逾期')}</div>
         </div>
         <div class="wbs-tip-box">
            <div class="wbs-tip-icon done"></div>
            <div class="wbs-tip-text">${this.translateWordPcc('已完成')}</div>
         </div>
         <div class="wbs-tip-box">
            <div class="wbs-tip-icon not-started-and-not-overdue">${this.translateWordPcc(
              '不显示'
            )}</div>
            <div class="wbs-tip-text">${this.translateWordPcc('未开始未逾期')}</div>
         </div>
      </div>`;
    }
    return `
      <p class="tip-content-title">${title}</p>
      ${content}
      `;
  }
