...
export class DroppableDirective implements OnInit {
  ...
  set disableDrop(val) {
    this._disableDrop = !!val;
    if (val) {
      this.el.nativeElement.classList.add('droppable-disabled');
    }
  }

  _disableDrop: boolean;
  /**
  ...
  allowDrop(ev, type?) {
    ...
    if (currentTarget.className === 'temp') {
      return false;
    }
    if (this.dragDropService.dragElement.contains(currentTarget)) {
      // 判断不是目标对象不是当前拖拽对象
      ref = false;
    } else if (currentTarget.classList.contains('connection')) {
      ref = false;
  ...
  dragOver(ev): void {
    if (
      this.dragDropService.dragElement.contains(ev.toElement) ||
      ev.toElement.contains(this.dragDropService.dragElement) ||
      ev.toElement.hasAttribute('tdroppable')
    ) {
      return;
    }
    ...
    if (this.allowDrop(ev, 'dragOver')) {
      ...
      } else if (positionY > 0.3 && positionY < 0.7) {
  ...
  dragLeave(ev) {
    const toElement = ev.toElement;
    if (
      this.dragDropService.dragElement.contains(toElement) ||
      toElement.contains(this.dragDropService.dragElement) ||
      toElement.hasAttribute('tdroppable')
    ) {
      return;
    }
  ...
  addTemp(ev, index) {
    // const nextSibling = ev.toElement.parentElement.nextElementSibling;
    // if (nextSibling && nextSibling?.hasAttribute('tdroppable')) {
    ...
    // }
    // if (this.dragDropService.positionType < 0) {
    //   this.insertBefore(this.dragDropService.tempElement, ev.toElement.parentElement);
    //   return;
    // }
    // if (this.dragDropService.positionType > 0) {
    //   this.insertAfter(this.dragDropService.tempElement, ev.toElement.parentElement);
    //   return;
    // }
    this.el.nativeElement.children[0].insertBefore(
      this.dragDropService.tempElement,
