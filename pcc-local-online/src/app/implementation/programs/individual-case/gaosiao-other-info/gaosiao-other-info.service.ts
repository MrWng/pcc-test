...
import { DwFormControl, dayjs, isEmpty } from '@athena/dynamic-core';

@Injectable()
export class GaosiaoOtherInfoService {
  atdmUrl: string;
  eocUrl: string;
  uibotUrl: string;
  smartDataUrl: string;
  ...
  dateTransForShow(current) {
    if (isEmpty(current)) {
      return '';
    }
    return dayjs(current).format('YYYY/MM/DD');
  }
  dateTrans(ctr) {
    if (!ctr.value || ctr['_prevValue'] === ctr.value) {
      return;
    }
    ctr.setValue(dayjs(ctr.value).format('YYYY-MM-DD'), {
      emitEvent: false,
    });
  }
