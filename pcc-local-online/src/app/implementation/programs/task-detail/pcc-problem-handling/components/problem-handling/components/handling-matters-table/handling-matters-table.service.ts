...
import { QuestDoStatus } from '../../../../config';
...
export class HandlingMattersTableService extends BaseDynamicCompBuilder {
  ...
  generateDynamicCop(pageData = [], editable = true) {
    ...
    return this.initDynamicTable({
      ...
      rules: this.createDynamicCopRulesService.createDynamicCopRules([
        {
          type: 'disabled',
          schema: 'is_conversion_task',
          targetSchema: 'uibot_checked',
          path: this.tableId,
          handler({ currentControl }) {
            const group = currentControl.parent;
            if (group.get('is_conversion_task').value) {
              group.get('uibot_checked').setValue(false);
            }
            return group.get('is_conversion_task').value;
          },
        },
      ]),
    });
