...
import { DmcUploadService, NzModalRef } from '@athena/dynamic-ui';
...
export class MaintenanceInformationComponent implements OnInit, OnChanges, OnDestroy {
  ...
  createGroup() {
    ...
    this.maintenanceInfoForm = new DwFormGroup({
      ...
      process_description: new DwFormControl(process_description, [Validators.required]),
  ...
  async removeFiles() {
    ...
    const promiseArr = [];
    const fileIdArr = this.delFileList.map((item) => item.id);
    ...
    this.delFileList.forEach((file) => {
      promiseArr.push(
        this.uploadAndDownloadService
          .tableInfoToAi('/api/aam/v1/deleteAttachment', {
    ...
    this.fileList = this.fileList.filter((item) => !fileIdArr.includes(item.id));
    return this.fileList;
  }
  /**
   * 选择人员异步校验
