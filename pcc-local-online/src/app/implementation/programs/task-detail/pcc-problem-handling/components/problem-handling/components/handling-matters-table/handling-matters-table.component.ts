...
export class HandlingMattersTableComponent implements OnInit, OnChanges, OnDestroy {
    ...
    this.problemHandlingService.pageDataChangeEmitter.subscribe(({ data, type }) => {
      ...
      switch (type) {
        case pageDataChangeEmitterType.INIT:
          ...
          this.triggerSchemaRule(
            this.handlingMattersTableService.tableGroup.get(
  ...
  triggerSchemaRule(control: DwFormArray, schema: string) {
    control.controls.forEach((ctr) => {
      ctr.get(schema).markAsDirty();
      ctr.get(schema).updateValueAndValidity();
    });
  }
}
