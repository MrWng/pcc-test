...
import { DwFormControl, DwFormGroup, isEmpty, isNotEmpty } from '@athena/dynamic-core';
import {
  ProblemExecutionMethod,
  QuestDoStatus,
  QuestPageDataStatus,
  QuestPageType,
} from '../../config';
...
@Component({
  ...
  providers: [ProblemHandlingService, AcModalService],
})
...
export class ProblemHandlingComponent implements OnInit, OnChanges, OnDestroy {
  ...
  get canBatchToTask(): boolean {
    return this.tableSelectRowsNeedTrans.length !== 0;
  }
  ...
  batchTransTaskLoading: boolean = false;
  addLoading: boolean = false;
  batchDelLoading: boolean = false;
  constructor(
  ...
  async batchTransTaskHandler(): Promise<any> {
    try {
      this.batchTransTaskLoading = true;
      const updateQuestionDoParams = [],
        employeeNameVerifyParams = [],
        publishTaskCarkParams = [];
      this.tableSelectRowsNeedTrans.forEach((transItem: DwFormGroup) => {
        const item = transItem.getRawValue();
        // 更新接口用的参数
        updateQuestionDoParams.push({
          ...item,
          actual_finish_datetime: isEmpty(item.actual_finish_datetime)
        ...
        // 发卡任务参数
        publishTaskCarkParams.push({
          question_no: item.question_no,
          question_do_no: item.question_do_no,
        });
        // 员工授权验证参数
        employeeNameVerifyParams.push(item);
      });
      const errInfo = await this.pccProblemHandlingService.employeeNameVerifyFn(
        employeeNameVerifyParams
      );
      ...
      await this.pccProblemHandlingService.updateQuestionDo(updateQuestionDoParams);
      await this.commonService
        .publishTaskCark(publishTaskCarkParams, 'PCC_mainline_project_0001')
        .toPromise();
      this.tableSelectRowsNeedTrans.forEach((group: DwFormGroup) => {
        group.get('is_conversion_task').setValue(true);
        // group.get('process_status').setValue(QuestDoStatus.TRANSFERRED_TASKS);
      });
      this.athMessageService.success(this.commonService.translatePccWord('转任务成功'));
    } catch (error) {
    } finally {
      this.batchTransTaskLoading = false;
      this.changeRef.markForCheck();
    }
  }
  selectRowChange(e) {
    this.tableSelectRows = e;
    this.tableSelectRowsNeedTrans = e.filter(
      (group: DwFormGroup) => group.get('process_status').value === QuestDoStatus.INCOMPLETION
    );
  }
