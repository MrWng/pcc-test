...
import {
  ...
  DynamicFormModel,
  cloneDeep,
  PostMessageDistributionService,
  WorkType,
...
export class QuestionQuickComponent extends DynamicFormControlComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if ((this.model as any).contents) {
      project_info = (this.model as any).contents.executeContext?.taskWithBacklogData?.bpmData
        ?.project_info;
      // 处理 112588 bug 新老表格 右上角按钮样式
      if (
        (this.model as any).contents.executeContext?.tmActivityId == 'apc_DTD_Assignment' ||
        (this.model as any).contents.executeContext?.tmActivityId == 'apco_DTD_Assignment'
      ) {
        this.isRight = true;
      } else {
        this.isRight = false;
      ...
      // 处理 112588 bug 新老表格 右上角按钮样式
      if (
        (this.model as any).content.executeContext?.tmActivityId == 'apc_DTD_Assignment' ||
        (this.model as any).content.executeContext?.tmActivityId == 'apco_DTD_Assignment'
      ) {
        this.isRight = true;
      } else {
        this.isRight = false;
  ...
  jumpFromCheck(): void {
    ...
    this.commonService.getInvData('bm.basc.app.auth.employee.get', params).subscribe(({ data }) => {
      ...
      if (employee.length === 0) {
        ...
        this.commonService.getInvData('bm.pisc.root.task.get', taskParams).subscribe(({ data }) => {
          ...
          if (task_info.length > 0) {
            ...
            this.modal.info({
              nzTitle: '提示',
              nzContent: this.translateService.instant('dj-default-无任务信息，请联系管理员！'),
              nzOkText: this.translateService.instant('dj-default-我知道了'),
              nzCancelText: null,
              nzClassName: 'confirm-modal-center-sty',
              nzOnOk: (): void => {
