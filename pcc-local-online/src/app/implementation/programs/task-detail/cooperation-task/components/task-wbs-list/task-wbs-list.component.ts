  ...
  implements OnInit, OnDestroy
{
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.datas = bpmData?.assist_schedule_info ? bpmData?.assist_schedule_info[0] : bpmData?.task_info[0];
    const datas = cloneDeep(this.datas);
    this.getSubmitBtnStatus(datas);
  ...
  getSubmitBtnStatus(datas) {
    if (datas && datas.project_no && datas.task_no) {
      ...
      this.commonService.getInvData('bm.pisc.assist.schedule.get', {
        assist_schedule_info: [ datas ],
      }).subscribe(res => {
        this.canClickSubmit = !!res?.data?.assist_schedule_info?.length;
  ...
  monitorTaskInfoGet(): void {
    this.wbsService.pageChange.subscribe((res) => {
      console.log('协同页面入口，重新获取页面数据：', res);
      this.getAssistTaskDetailInfo();
    });
  }
