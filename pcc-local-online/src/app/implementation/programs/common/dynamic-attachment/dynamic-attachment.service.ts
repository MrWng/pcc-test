...
@Injectable()
export class DynamicAttachmentService {
  atdmUrl: string;
  eocUrl: string;
  uibotUrl: string;
  smartDataUrl: string;
  ...
  filterFileTypeByName(name: string = '') {
    const excelExec = ['xls', 'xlsx'],
      pafExec = ['pdf'],
      imgExec = ['BMP', 'jpg', 'jpeg', 'png', 'gif'];
    if (excelExec.includes(name.toLowerCase())) {
      return 'excel';
    }
    if (pafExec.includes(name.toLowerCase())) {
      return 'pdf';
    }
    if (imgExec.includes(name.toLowerCase())) {
      return 'img';
    }
  }
  /**
