import { Component, Input, OnInit, ChangeDetectorRef, ElementRef,
  Output, EventEmitter, AfterViewInit, ViewEncapsulation, OnDestroy } from '@angular/core';
...
import { CustomOpenWindowService } from '@app-custom/ui/open-window';
...
@Component({
  ...
  providers: [ReportProjectQuestionService, CommonService, CustomOpenWindowService]
})
...
export class ReportProjectQuestionComponent extends DynamicFormControlComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  actionId?: any = 'bm.pisc.project.get';
  apiParams?: any = {
    search_info: [{
  ...
  operations = {
    useHasNext: false,
    roleAttention: [ 'project_no', 'project_name', 'project_leader_code', 'project_leader_name' ],
    dataKeys: ['project_name', 'project_no'],
    backFills: [
      {
        valueScript: "selectedObject['project_name']",
        key: 'project_name'
      },
      {
      ...
      {
        valueScript: "selectedObject['project_leader_name']",
        key: 'project_leader_name'
      },
      {
        valueScript: "selectedObject['project_leader_code']",
        key: 'project_leader_code'
      },
    ]
  };
  ...
  constructor(
    ...
    private customOpenWindowService: CustomOpenWindowService,
  ) {
  ...
  getTableData(isInit: boolean) {
    ...
    const params = [];
    if (this.validateForm?.value?.projectInfos?.length) {
      this.validateForm?.value?.projectInfos?.forEach(item =>{
        params.push({
          app_project_no: item.project_no, // 应用项目编号
      ...
      params.push({
        app_project_no: null, // 应用项目编号
        question_type_no: this.validateForm.value.questionType?.question_type_no ?? null, // 问题类型编号
        process_person_no: this.validateForm.value.questionLeader?.employee_no ?? null,// 发起人编号
        initiator_no: this.validateForm.value.questionProposer?.employee_no ?? null // 处理人编号
      });
    }
    this.commonService.getInvData('bm.basc.project.question.list.get', { search_info, question_list_info: params })
      .subscribe((res): void => {
